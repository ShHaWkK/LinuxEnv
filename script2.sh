...
# Export existing GPG keys from the keyring to the vault
gpg_export(){
  log "== GPG EXPORT =="
  ensure_env_open || return
  mapfile -t keys < <(gpg --list-secret-keys --with-colons | awk -F: '/^sec/ {print $5}')
  [[ ${#keys[@]} -eq 0 ]] && { whiptail --msgbox "Aucune clé disponible" 8 50; return; }
  ...
  chown "$OWNER":"$OWNER" "$GPG_DIR/public_${CH}.gpg"
  if whiptail --yesno "Exporter la clé privée ?" 8 50; then
    gpg --export-secret-keys --armor "$CH" >"$GPG_DIR/private_${CH}.gpg"
    chmod 600 "$GPG_DIR/private_${CH}.gpg"
    chown "$OWNER":"$OWNER" "$GPG_DIR/private_${CH}.gpg"
  fi
  local msg="✅ Clé exportée"
  success "$msg"
  show_summary "$msg"
}
...
ssh_start(){
  ...
  tags=(); for f in "${cfgs[@]}"; do
    host="$(basename "$f" | sed 's/^sshconf_//')"
    tags+=( "$host" "" )
  done
