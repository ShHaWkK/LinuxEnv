...
for cmd in cryptsetup mkfs.ext4 mount umount fallocate dd losetup lsblk df blkid gpg ssh-keygen whiptail; do
...
unlock(){ printf '%s' "$PASS" | cryptsetup open --type luks1 --key-file=- "$1" "$MAPPER"; }
lock(){ cryptsetup close "$MAPPER"; }
mount_env(){ mount "/dev/mapper/$MAPPER" "$MOUNT"; }
umount_env(){ umount "$MOUNT" &>/dev/null || :; }
...
install_env(){
  ...
  info "Format LUKS (tapez YES)"; printf '%s' "$PASS" | cryptsetup luksFormat --batch-mode "$LOOPDEV" --key-file=- & spinner $!; show_lsblk
  info "Ouverture volume"; unlock "$LOOPDEV"; show_lsblk
  info "Format ext4"; mkfs.ext4 "/dev/mapper/$MAPPER" & spinner $!; show_lsblk
  info "Montage"; mount_env; chmod -R go-rwx "$MOUNT"; show_lsblk; show_df
  success "Env installé et monté ➜ $MOUNT"
}
open_env(){
  info "OUVERTURE ENV"; show_lsblk
  [[ ! -f "$CONTAINER" ]] && { error "Pas de conteneur"; return; }
  [[ ! -f "$LOOP_FILE" ]] && attach_loop
  [[ ! -e "/dev/mapper/$MAPPER" ]] && { read -s -p "Passphrase LUKS : " PASS; echo; unlock "$(cat "$LOOP_FILE")"; }
  mount_env; chmod -R go-rwx "$MOUNT"; show_lsblk; show_df; success "Env ouvert"
}
close_env(){
  info "FERMETURE ENV"; umount_env && success "Démonté"
  [[ -e "/dev/mapper/$MAPPER" ]] && lock && success "Verrouillé"
  detach_loop && success "Loop détaché"; show_lsblk
}
delete_env(){
  info "SUPPRESSION ENV"; close_env; rm -f "$CONTAINER"; rmdir "$MOUNT" &>/dev/null || :; show_lsblk; success "Env supprimé"
}
backup_env(){
...
gpg_setup(){
  info "GPG GENERATE"; read -p "Nom : " N; read -p "Email : " E; read -p "Commentaire : " C
  cat >gpg-batch <<EOF
%no-protection
Key-Type: default
Subkey-Type: default
...
gpg_import(){
  info "IMPORT GPG"; for f in "$GPG_DIR"/*.gpg; do gpg --import "$f" && success "Importé $f"; done
}
gpg_export(){
  info "EXPORT GPG"; for key in $(gpg --list-secret-keys --with-colons|awk -F: '/^sec/ {print $5}'); do
    gpg --export --armor "$key" >"$GPG_DIR/public_$key.gpg"
    gpg --export-secret-keys --armor "$key" >"$GPG_DIR/private_$key.gpg"
    chmod 600 "$GPG_DIR/private_$key.gpg"
    chmod 644 "$GPG_DIR/public_$key.gpg"
...
ssh_create_tpl(){
  ...
  success "SSH config ➜ sshconf_$H + clé copiée"
}
ssh_setup_alias(){
...
ssh_delete(){
  info "SSH DELETE"; rm -rf "$SSH_DIR"/sshconf_* "$SSH_DIR"/$(basename *) &>/dev/null || :; success "SSH supprimé"
}
ssh_backup(){
  info "SSH BACKUP"; ts=$(date +%Y%m%d_%H%M%S)
  tar czf "$SSH_BACKUP_DIR/ssh_wallet_$ts.tar.gz" -C "$SSH_DIR" .; success "SSH backup ➜ $SSH_BACKUP_DIR"
}
restore_ssh_wallet(){
  info "SSH RESTORE"
  ...
  tar xzf "$SSH_BACKUP_DIR/$B" -C "$SSH_DIR"; success "Restauré ➜ $B"
}
auto_open_toggle(){
  info "AUTO-OPEN TOGGLE"
  if [[ -f "$AUTOOPEN_FLAG" ]]; then
    sed -i "\|env.sh open_env|d" ~/.bashrc; rm -f "$AUTOOPEN_FLAG"; success "Auto-open désactivé"
  else
    echo "$PWD/env.sh open_env" >>~/.bashrc; touch "$AUTOOPEN_FLAG"; success "Auto-open activé"
  fi
}
...
main_menu(){
  while :; do
    CH=$(whiptail --title "Coffre Sécurisé" --menu "Section" 15 60 4 \
      1 "Environnement" 2 "GPG" 3 "SSH" 4 "Quitter" \
      3>&1 1>&2 2>&3)||exit
    case $CH in
      1) env_menu ;;
      2) gpg_menu ;;
