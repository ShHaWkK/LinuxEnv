...
# log file in non-root user home for permission safety
...
# ─── Affichage du résumé et des derniers logs ─────────────────────────────────
show_summary(){
  local msg="${1:-}"
...
# ─── Nettoyage d’éventuels mounts/mappers précédents ──────────────────────────
cleanup_stale(){
...
# ─── Ouvre+monte l’environnement si nécessaire ────────────────────────────────
ensure_env_open(){
  if ! mountpoint -q "$MOUNT"; then
    open_env || return 1
  fi
...
open_env(){
  ...
  mkdir -p "$SSH_DIR" "$GPG_DIR"; log "[OK] Monté sur $MOUNT"
...
EOF
  ...
  gpg --export --armor "$key" >"$GPG_DIR/public_${key}.gpg"; log "[OK] pub → $GPG_DIR/public_${key}.gpg"
...
ssh_create_template(){
  ...
  ensure_env_open || return
  [[ ! -f "$SSH_CONFIG" ]] && { whiptail --msgbox "Pas de $SSH_CONFIG" 6 50; log "[ER] pas de config"; return; }
  mapfile -t hosts < <(grep '^Host ' "$SSH_CONFIG" | awk '{print $2}')
  [[ ${#hosts[@]} -eq 0 ]] && { whiptail --msgbox "Aucun host" 6 50; log "[ER] hosts=0"; return; }
  ...
  local idf=$(awk "/^Host $CH$/,/^Host /" "$SSH_CONFIG" | awk '/IdentityFile/ {print $2;exit}')
  if [[ -n "$idf" ]]; then
    cp "$idf" "$SSH_DIR/"; chmod 600 "$SSH_DIR/$(basename "$idf")"
    sed -i "s|IdentityFile .*|IdentityFile $SSH_DIR/$(basename "$idf")|" "$SSH_DIR/sshconf_$CH"
  fi
  log "[OK] sshconf_$CH → $SSH_DIR/sshconf_$CH"
  local msg="✅ SSH template créé → $SSH_DIR/sshconf_$CH"
  success "$msg"; show_summary "$msg"
}
...
ssh_import_host(){
  log "== SSH IMPORT HOST =="
  ensure_env_open || return
  [[ ! -f "$SSH_CONFIG" ]] && { whiptail --msgbox "Pas de $SSH_CONFIG" 6 50; log "[ER] no config"; return; }
  mapfile -t hosts < <(grep '^Host ' "$SSH_CONFIG" | awk '{print $2}')
  [[ ${#hosts[@]} -eq 0 ]] && { whiptail --msgbox "Aucun host" 6 50; log "[ER] none"; return; }
  tags=(); for h in "${hosts[@]}"; do tags+=( "$h" "" ); done
  CH=$(whiptail --menu "Choisissez host" 15 60 ${#hosts[@]} "${tags[@]}" 3>&1 1>&2 2>&3) || return
  awk "/^Host $CH$/,/^Host /" "$SSH_CONFIG" >"$SSH_DIR/sshconf_$CH"
  local idf=$(awk "/^Host $CH$/,/^Host /" "$SSH_CONFIG" | awk '/IdentityFile/ {print $2;exit}')
...
ssh_delete(){
  ...
  ensure_env_open || return
  rm -rf "$SSH_DIR"/*
  whiptail --msgbox "Vault SSH vidé" 6 50; log "[OK] vidé"
...
restore_ssh_wallet(){
  ...
  [[ ${#bs[@]} -eq 0 ]] && { whiptail --msgbox "Pas de backup SSH" 6 50; log "[ER] none"; return; }
...
if [[ "${1:-}" == "--menu" ]]; then
  ...
  while true; do
    CH=$(whiptail --title "Coffre Sécurisé" --menu "Section" 15 60 4 \
      ...
      Cryptographie   "GPG" \
      SSH             "SSH" \
      Quitter         "Quitter" 3>&1 1>&2 2>&3) || exit
    case $CH in
      Environnement)
