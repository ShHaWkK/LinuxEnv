...
if [[ -n "${SUDO_USER-}" && "$SUDO_USER" != "root" ]]; then
  USER_HOME="/home/$SUDO_USER"; OWNER="$SUDO_USER"
else
  USER_HOME="$HOME"; OWNER="$(id -un)"
fi

# ─── Log unique dans le home de l’utilisateur ─────────────────────────────────
LOG="$USER_HOME/secure_env.log"
: >"$LOG"; chmod 600 "$LOG"; chown "$OWNER":"$OWNER" "$LOG"
exec 3>&1
log(){ echo "[$(date +%T)] $*" >>"$LOG"; }
...
chown -R "$OWNER":"$OWNER" "$MOUNT" "$BACKUP"

# ─── Petite animation pour les longues opérations ─────────────────────────────
spinner(){
  local pid=$1 sp='|/-\' i=0
...
# ─── Affiche les 10 derniers logs + message via Whiptail ───────────────────────
show_summary(){
  local msg="${1:-}"
...
# ─── Nettoyage des mappers/mounts éventuels ────────────────────────────────────
cleanup_stale(){
...
install_env(){
  ...
  chmod 600 "$CONTAINER"; chown "$OWNER":"$OWNER" "$CONTAINER"; log "[OK] créé"
...
open_env(){
  ...
  [[ ! -f "$CONTAINER" ]] && { log "[ER] conteneur manquant"; show_summary "❌ manquant"; return; }
  if ! cryptsetup status "$MAPPER" &>/dev/null; then
    read -s -p "Passphrase LUKS : " PASS; echo
    info "Ouverture LUKS…"
    printf '%s' "$PASS" \
    ...
    info "⚠️ déjà ouvert"; log "[!!] déjà ouvert"
  fi
  if ! mountpoint -q "$MOUNT"; then
    mount "/dev/mapper/$MAPPER" "$MOUNT"
    chmod -R go-rwx "$MOUNT"; chown -R "$OWNER":"$OWNER" "$MOUNT"
  ...
  success "✅ Env ouvert et monté"; show_summary
}

close_env(){
  log "== CLOSE ENV =="
  mountpoint -q "$MOUNT" && { umount "$MOUNT"; log "[OK] démonté"; } || info "⚠️ pas monté"
  cryptsetup status "$MAPPER" &>/dev/null && { cryptsetup close "$MAPPER"; log "[OK] fermé"; } \
    || info "⚠️ déjà fermé"
  success "✅ Env fermé"; show_summary
}

delete_env(){
  log "== DELETE ENV =="
  close_env
  [[ -f "$CONTAINER" ]] && { rm -f "$CONTAINER"; log "[OK] conteneur supr."; }
  rmdir "$MOUNT" &>/dev/null || :
  success "✅ Env supprimé"; show_summary
}
...
gpg_export(){
  ...
  (( ${#keys[@]} )) || { whiptail --msgbox "Aucune clé" 6 50; return; }
  ...
  if whiptail --yesno "Exporter la privée ?" 8 50; then
...
# Exemple de config SSH utilisable avec -F
Host monserveur
    HostName example.com
    User utilisateur
    Port 22
    IdentityFile /chemin/vers/clef
EOF
  ...
  success "✅ Template SSH"; show_summary
}
...
ssh_setup_alias(){
  ...
  success "✅ Alias SSH"; show_summary
}
...
ssh_import_host(){
  ...
  keyfile=$(awk "/^Host $CH$/,/^Host /" "$SSH_CONFIG" | awk '/IdentityFile/{print $2;exit}')
  if [[ -f "$keyfile" ]]; then
...
ssh_start(){
  ...
  (( ${#cfgs[@]} )) || { whiptail --msgbox "Aucune config dans coffre" 6 50; return; }
  ...
  success "✅ SSH session $CH terminée"; show_summary
}

ssh_delete(){
  log "== SSH DELETE =="
  ensure_env_open || return
  rm -f "$SSH_DIR"/sshconf_* "$SSH_DIR"/ssh_template.conf "$ALIAS_FILE"
...
ssh_backup(){
  ...
  whiptail --msgbox "Backup créé : ssh_wallet_${ts}.tar.gz" 6 60
  success "✅ SSH backup"; show_summary
}

restore_ssh_wallet(){
  log "== SSH RESTORE =="
  ensure_env_open || return
  mapfile -t presses < <(ls "$SSH_BACKUP_DIR"/ssh_wallet_*.tar.gz 2>/dev/null)
  (( ${#presses[@]} )) || { whiptail --msgbox "Aucune sauvegarde" 6 50; return; }
  tags=(); for b in "${presses[@]}"; do tags+=( "$(basename "$b")" "" ); done
  CH=$(whiptail --menu "Choisissez backup" 15 60 ${#presses[@]} "${tags[@]}" 3>&1 1>&2 2>&3) || return
  tar xzf "$SSH_BACKUP_DIR/$CH" -C "$SSH_DIR"
  chown -R "$OWNER":"$OWNER" "$SSH_DIR"
  success "✅ SSH restauré"; show_summary
}
